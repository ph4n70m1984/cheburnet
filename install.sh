...
trap 'stty echo 2>/dev/null; printf "\n${R}[!] Прервано пользователем.${N}\n"; [ -n "$WORK_DIR" ] && rm -rf "$WORK_DIR"; [ "$SERVICE_STOPPED" = "1" ] && /etc/init.d/"$SERVICE_NAME" start >/dev/null 2>&1 || true; exit 1' INT TERM[cite: 15]
...
unset _READ_TIMEOUT_TEST[cite: 15]
...
TARGET_PACKAGE_ARCH="${APK_SYSTEM_ARCH:-$DISTRIB_ARCH}"[cite: 15]
...
printf "  Текущий Chebur.NET:   ${Y}%s${N}\n\n" "${CURRENT_CHEBUR_VER:-не установлен}"[cite: 15]
...
read_input 30
CHOICE_SB="${READ_VALUE:-1}"[cite: 15]
...
[ -z "$CHEBUR_RELEASE_JSON" ] && fail "Не удалось получить метаданные релиза Chebur.NET."[cite: 15]
...
if [ "$NEED_UPDATE_CHEBUR" = "1" ]; then
    ...
    if [ "$PKG_MANAGER" = "apk" ]; then
        ...
        [ -z "$CHEBUR_URL" ] && fail "Не найден подходящий .apk пакет для $MATCH_ARCH."[cite: 15]

        printf "${C}[*] Скачивание %s...${N}\n" "$(basename "$CHEBUR_URL")"
        $DOWNLOAD "/tmp/cheburnet.apk" "$CHEBUR_URL" || fail "Сбой при скачивании cheburnet.apk"[cite: 15]
        ...
        rm -f /tmp/cheburnet.apk /.PKGINFO /.pre-install /.post-install 2>/dev/null || true[cite: 15]
    else
        ...
        [ -z "$CHEBUR_URL" ] && fail "Не найден .ipk пакет под архитектуру $CHEBUR_ARCH."[cite: 15]

        printf "${C}[*] Скачивание %s...${N}\n" "$(basename "$CHEBUR_URL")"
        $DOWNLOAD "/tmp/cheburnet.ipk" "$CHEBUR_URL" || fail "Сбой при скачивании cheburnet.ipk"[cite: 15]

        printf "${C}[*] Установка пакета через opkg...${N}\n"
        opkg install /tmp/cheburnet.ipk --force-reinstall
        rm -f /tmp/cheburnet.ipk[cite: 15]
    fi
